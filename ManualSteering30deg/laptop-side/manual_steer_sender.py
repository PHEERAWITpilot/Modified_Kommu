...
# Two-tier angle caps. Panda hard ceiling = 45 deg (byd.h max_angle=450 raw).
# NORMAL: 20 deg — 25 deg margin to 45 deg ceiling.
# HIGH:   30 deg — 15 deg margin to 45 deg ceiling. Deliberate intermediate
#         step (not 40) — validate 30 before approaching the safety ceiling.
# HIGH is ONLY reachable via the H key toggle when OVERRIDE (M) is already ON.
# The CLI --max-angle arg is clamped to NORMAL (20 deg); HIGH is runtime-only
# and cannot be raised via any command-line flag.
ANGLE_CAP_NORMAL     = 20.0
ANGLE_CAP_HIGH       = 30.0
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key in (pygame.K_q, pygame.K_ESCAPE):
                        ...
                        if not override_active:
                            ...
                            print(f"[sender] HIGH RANGE {'ON (±30°)' if high_range_active else 'OFF — back to NORMAL (±20°)'}")
...
def main():
    ...
    # (byd.h max_angle=450 raw). All three layers (sender/writer/hook) now
    # agree at 20 deg — this refusal ceiling is the last line of defence, not
    # one of several progressively-tighter clamps. args.max_angle == 20.0
    # (the new default) passes this check (strict >), since 20.0 is not
    # greater than HARD_MAX_ANGLE_DEG (20.0); only values ABOVE 20 are refused.
