...
# NORMAL: 20 deg — 25 deg margin to Panda max_angle 45 deg (byd.h max_angle=450 raw).
# HIGH:   30 deg — 15 deg margin to Panda max_angle 45 deg. Deliberate intermediate
#         step: validate 30 before approaching ceiling. A future session may widen
#         to 40 only after 30 is verified, preserving >=5 deg margin before ceiling.
...
MANUAL_ABS_MAX_DEG_NORMAL = 20.0
MANUAL_ABS_MAX_DEG_HIGH   = 30.0
...
def _read_target(now, target_file=TARGET_FILE, stale_s=STALE_S):
  ...
  selects the cap tier: False -> NORMAL (20 deg), True -> HIGH (30 deg).
