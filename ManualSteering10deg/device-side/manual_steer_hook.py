...
SPEED_FLOOR_MS = 0.30
# Hard clamp on the manual DESIRED angle before it enters the limit chain.
# This is a *desired* clamp; the device limit chain clamps again to +/-10 deg
# of measured and to STEER_ANGLE_MAX. Keep this conservative for bench/first use.
MANUAL_ABS_MAX_DEG = 10.0


def _read_target(now, target_file=TARGET_FILE, stale_s=STALE_S):
  """Return a fresh manual angle (float) or None if missing/stale/invalid."""
  try:
...
def manual_desired_angle(planner_angle, lat_active, v_ego, *,
  ...
  angle = _read_target(now, target_file=target_file, stale_s=stale_s)
  if angle is None:
    return planner_angle, False

  # Clamp the manual DESIRED angle (device limit chain clamps again downstream).
  angle = _clamp(float(angle), -abs_max_deg, abs_max_deg)
  return angle, True
